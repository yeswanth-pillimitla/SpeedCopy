...
const orderChannelsData = [
  { name: 'Direct', value: 42, color: '#111827' },
  { name: 'Vendor Store', value: 31, color: '#5850EC' },
  { name: 'Search', value: 18, color: '#8A94A6' },
  { name: 'Referral', value: 9, color: '#E5E7EB' }
]
...
const auditActivities = [
  {
    title: 'Price updated',
    description: 'Offer price set to ₹19.99',
    user: 'by Admin',
    time: '2 hours ago'
  },
  {
    title: 'Stock updated',
    description: 'Inventory restocked: +1000 units',
    user: 'by Vendor: PrintHub',
    time: '1 day ago'
  },
  {
  ...
  },
  {
    ...
    description: 'PRD-1029 added to catalog',
    user: 'by Admin',
    time: 'Jan 12, 2026'
  }
]
...
export default function PremiumBusinessCard() {
  useEffect(() => {
    document.title = 'Premium Business Cards 300gsm -SpeedCopy'
  }, [])
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'Overview' && (
          <motion.div
            ...
            className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 shrink-0"
          >
            {/* LEFT COLUMN */}
        ...
        {activeTab === 'Analytics' && (
          <motion.div
            ...
            animate="show"
            exit={{ opacity: 0, y: -15 }}
            className="w-full pt-[20px] flex flex-col gap-6 shrink-0"
          >
            {/* TOP CARDS GRID */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
              ...
              <motion.div
                variants={cardVariants}
                className="col-span-12 lg:col-span-8 p-5 bg-white border border-[#E2E8F0] rounded-[14px] shadow-[0px_1px_2px_-1px_rgba(0,0,0,0.1),0px_1px_3px_0px_rgba(0,0,0,0.1)] h-[333.55px] flex flex-col justify-between"
              >
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-[14px] font-bold text-gray-900 font-sans">Orders & Revenue</h3>
                  <span className="text-[11px] text-slate-400 font-bold font-sans">Jan – Jun 2026</span>
                </div>
                <div className="w-full h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={salesData} margin={{ top: 10, right: -5, left: -25, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F1F5F9" />
                      <XAxis
                        dataKey="name"
                        axisLine={false}
              ...
              <motion.div
                variants={cardVariants}
                className="col-span-12 lg:col-span-4 p-5 bg-white border border-[#E2E8F0] rounded-[14px] shadow-[0px_1px_2px_-1px_rgba(0,0,0,0.1),0px_1px_3px_0px_rgba(0,0,0,0.1)] h-[333.55px] flex flex-col justify-between"
              >
                <div className="flex justify-between items-center mb-1">
                  <h3 className="text-[14px] font-bold text-gray-900 font-sans">Order Channels</h3>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        animationDuration={1200}
                      >
                        {orderChannelsData.map((entry, index) => (
                ...
                <div className="flex flex-col gap-2 mt-2 px-1">
                  {orderChannelsData.map((channel) => (
                    <div key={channel.name} className="flex justify-between items-center text-[12px] font-sans">
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: channel.color }} />
                        <span className="text-slate-550 font-semibold">{channel.name}</span>
                      </div>
                      <span className="text-gray-900 font-bold">{channel.value}%</span>
                    </div>
                  ))}
                </div>
              </motion.div>
              ...
              {bottomStats.map((card) => {
                ...
                return (
                  <motion.div
                    key={card.title}
                    variants={cardVariants}
                    className="p-5 bg-white border border-[#E2E8F0] rounded-[14px] shadow-[0px_1px_2px_-1px_rgba(0,0,0,0.1),0px_1px_3px_0px_rgba(0,0,0,0.1)] h-[117.6px] flex flex-col justify-between"
                  >
                    <div className="flex justify-between items-start">
                      <span className="text-[12px] font-semibold text-slate-400 font-sans leading-none">{card.title}</span>
                      <CardIcon className="w-4 h-4 text-slate-300" />
                    </div>
                    <div className="flex flex-col gap-0.5 mt-2">
                      <span className="text-[22px] font-black text-gray-900 leading-none font-sans">{card.value}</span>
                      <span className="text-[11px] font-medium text-slate-400 leading-none mt-1 font-sans">{card.subtext}</span>
                    </div>
                  </motion.div>
                )
              })}
        ...
        {activeTab === 'Activity' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -15 }}
            transition={{ duration: 0.25 }}
            className="w-full pt-[20px] shrink-0"
          >
            <div className="w-full bg-white border border-[#E2E8F0] rounded-[14px] shadow-[0px_1px_2px_-1px_rgba(0,0,0,0.1),0px_1px_3px_0px_rgba(0,0,0,0.1)] p-6 flex flex-col font-sans">
              {/* Header Section */}
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-[16px] font-bold text-gray-900">Audit Log</h3>
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => alert('Log exported successfully!')}
                  className="flex items-center gap-1.5 text-[12px] font-bold text-slate-400 hover:text-black transition-colors cursor-pointer"
                >
              ...
              >
                {auditActivities.map((act, index) => {
                  const isLast = index === auditActivities.length - 1
                  return (
                    <motion.div
                      key={index}
                      variants={cardVariants}
                      className={`flex items-start justify-between py-4 ${!isLast ? 'border-b border-[#F1F5F9]' : ''}`}
                    >
                      <div className="flex gap-4">
                        {/* Circular Icon */}
                        <div className="w-9 h-9 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 shrink-0 mt-0.5">
                          <FiClock className="w-4 h-4 text-slate-400" />
                        </div>
                        {/* Middle Details */}
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[14px] font-bold text-gray-900">{act.title}</span>
                          <span className="text-[12.5px] text-slate-500 font-medium">{act.description}</span>
                          <span className="text-[11px] text-slate-400 font-bold mt-0.5">{act.user}</span>
                        </div>
                      </div>
                      {/* Time */}
                      <span className="text-[11px] text-slate-400 font-bold mt-1">{act.time}</span>
                    </motion.div>
                  )
                })}
              </motion.div>
